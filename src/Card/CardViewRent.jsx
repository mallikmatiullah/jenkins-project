...
import { Box, Flex, Grid, Image, Text, Button, Icon, Center } from "@chakra-ui/react";
...
export default function CardViewRent({ property }) {
    const navigate = useNavigate();

    const [favorites, setFavorites] = useState(() => {
        const savedFavorites = localStorage.getItem("rentFavorites");
        return savedFavorites ? JSON.parse(savedFavorites) : {};
    });

    useEffect(() => {
        localStorage.setItem("rentFavorites", JSON.stringify(favorites));
    }, [favorites]);
    ...
    const titleclick = (id) => {
        navigate(`/rent-detail-product-page/${id}`);
    }
        ...
        <Grid templateColumns="repeat(auto-fit, minmax(250px, 1fr))" gap={1}>
            <Box mt={16} px={[4, 8, 16, 20]}>
                ...
                >
                    ...
                    >
                        <Image
                            src={property.images[0] ? getRentImagesUrl(property.id, property.images[0]) : ""}
                            alt="property image"
                            borderTopRadius="lg"
                            w="100%"
                            h="200px"
